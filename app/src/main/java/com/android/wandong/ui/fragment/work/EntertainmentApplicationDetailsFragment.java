...
public class EntertainmentApplicationDetailsFragment extends ABaseFragment {
    ...
    private void populateView(EntertainApplyDetailResponseBean result) {
        if (result.getEntityInfo().getDetail() != null) {
            ...
            Tools.setTextView(mViewmoneyr, mMoneyFormat.format(result.getEntityInfo().getDetail().getEstimateTotal()));
        ...
        if (result.getEntityInfo().getApproval() != null) {
            for (EntertainApplyDetailResponseBean.EntityInfoBean.ApprovalBean bean : result.getEntityInfo().getApproval()) {
